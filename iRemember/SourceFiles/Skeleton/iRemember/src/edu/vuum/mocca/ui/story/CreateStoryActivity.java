...
public class CreateStoryActivity extends StoryActivityBase {
	...
	private void launchSoundIntent() {
		...
		Intent newIntent = new Intent(CreateStoryActivity.this,
		...
		newIntent.putExtra(SoundRecordActivity.EXTRA_OUTPUT,
				filename.getAbsolutePath());

		// TODO - Start a new activity for result, using the new intent and the
		// request
		// code MIC_SOUND_REQUEST
		this.startActivityForResult(newIntent, MIC_SOUND_REQUEST);
	...
	private void launchCameraIntent() {
		...
		newIntent.putExtra(MediaStore.EXTRA_OUTPUT, fragment.imagePath);
		...
		// TODO - Start a new activity for result, using the new intent and the
		// request
		// code CAMERA_PIC_REQUESTS
		this.startActivityForResult(newIntent, CAMERA_PIC_REQUEST);
	...
	private void launchVideoCameraIntent() {
		...
		// See:
		// http://developer.android.com/reference/android/provider/MediaStore.html
		Intent myintnet = new Intent(MediaStore.ACTION_VIDEO_CAPTURE);
		...
		// Intent-extra name
		// from the MediaStore class, EXTRA_OUTPUT
		myintnet.putExtra(MediaStore.EXTRA_OUTPUT, filename.getAbsolutePath());
		...
		myintnet.putExtra(MediaStore.EXTRA_VIDEO_QUALITY, 1);

		// TODO - Start a new activity for result, using the new intent and the
		// request
		// code CAMERA_VIDEO_REQUEST
		this.startActivityForResult(myintnet, CAMERA_VIDEO_REQUEST);
